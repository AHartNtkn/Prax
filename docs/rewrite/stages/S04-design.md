...
Builder idiom per ARCHITECTURE: `Action::new(label).when([...]).then([...])`, `Practice::new(id).roles([...]).action(...).data_facts([...]).init([...])`, `Function::new(name, params).case(when, then)`, `Character::new(name).want(...).holds(...)`, `ScheduleRule::new(name, period).clause(when, then)`. Builders take `impl Into<String>` / `impl IntoIterator`; they are INFALLIBLE — they build values. Free outcome constructors join S2's condition surface: `insert(s)`, `delete(s)`, `insert_for(n, s)`, `call(f, args)`, `for_each(when, then)`; `Roll` is built only by `rng::draw` on the guarded path (the variant stays public so the unseeded-Roll pin can construct one).
...
struct Runtime {  // plain clone; tries share structurally
    ...
    expiries: BTreeMap<CompiledPath, i64>,  // exact labeled path → due boundary
    rng_seed: Option<i64>,
}
```
...
**obligedClose adjudication (owed row): discharge at S4** via a minimal `prax_vocab::deontic` (OBLIGED_HEAD, obligation_path, OBLIGED_LIFT_PREFIX, obliged_lift, obliged_close — the constants S9 needs anyway). Pin: closing over `obliged_close(axs)` derives the sub-obligation; bare closure does NOT lift. Honest caveat, panel-flagged: prax-vocab is born early with one module; the alternative (obliged_close in prax-core) violates the crate charter and is rejected.
...
Interner (settled, panel-flagged): perform/possible_actions take `&mut self`; `Arc::make_mut` on the append-only interner is fork-safe (pre-clone ids stable; post-clone ids fork-local; no id observable).
...
- EngineSpec: everything EXCEPT the two groundedDeltaAnchors pins (owed: S6). Build-order pin on compiled-rule equality (typeCheck clause noted for S9).
