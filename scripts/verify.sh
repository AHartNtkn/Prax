...
#   3. cabal test            — the frozen Haskell suite stays green (the pins' source)
#   4. cargo build/clippy/test (workspace) — the Rust side compiles clean, no warnings
#
...
step "1/4 freeze-check"
./scripts/freeze-check.sh

step "2/4 cabal build prax-oracle (additive oracle surface)"
cabal build prax-oracle

step "3/4 cabal test (frozen Haskell suite must stay green)"
cabal test

step "4/4 cargo build + clippy + test (Rust workspace)"
